...
- [x] Browser pool size is configurable.
- [x] Scraping work borrows browser instances from the pool and returns them after use.
- [x] Browser instances are closed cleanly when a run completes or fails.
- [x] Pool behavior is bounded so configured concurrency is not exceeded.
- [x] Tests cover pool acquisition, release, cleanup, and failure paths without requiring real browsers.
