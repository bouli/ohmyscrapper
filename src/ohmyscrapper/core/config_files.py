...
def create_and_read_config_file(file_name, default_app_dir, force_default=False):
    ...
    if force_default or not os.path.exists(config_file):
        ...
        overwrite_config_file(data=config_params ,file_name=file_name, default_app_dir=default_app_dir)
    else:
        with open(config_file, "r") as f:
            config_params = yaml.safe_load(f.read())
    if config_params is None:
        config_params = create_and_read_config_file(file_name=file_name, default_app_dir=default_app_dir, force_default=True)
