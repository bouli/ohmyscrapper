...
def sniff_url(url='https://www.linkedin.com/in/cesardesouzacardoso/', silent=False):
    ...
    report_meta_tags = []
    tags_to_search = ['description', 'og:url', 'og:title', 'og:description', 'og:type', 'lnkd:url']
    ...
    for meta_tag in soup.find_all("meta"):
        if meta_tag.get("name") in tags_to_search or meta_tag.get("property") in tags_to_search:
    ...
    for report_meta_tag in report_meta_tags:
        if report_meta_tag.get("name") is not None:
            ...
            final_report[report_meta_tag.get("property")] = report_meta_tag.get("content")
